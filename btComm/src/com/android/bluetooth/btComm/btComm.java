package com.android.bluetooth.btComm;
...
public class btComm {
    ...
    public static final int MESSAGE_BROKEN = 6;
    ...
    private synchronized void setState(int state) {
    	...
    	mHandler.obtainMessage(btComm.MESSAGE_STATE_CHANGE, state, -1).sendToTarget();
    }
    ...
    private void connectionFailed() {
        ...
        Message msg = mHandler.obtainMessage(btComm.MESSAGE_TOAST);
        Bundle bundle = new Bundle();
        //bundle.putString(btComm.TOAST, "Unable to connect device");
        msg.setData(bundle);
        mHandler.sendMessage(msg);
    }
    ...
    private void connectionLost() {
        ...
        // Send a failure message back to the Activity
        Message msg = mHandler.obtainMessage(btComm.MESSAGE_BROKEN);
        mHandler.sendMessage(msg);
    }
        ...
        public void run() {
            ...
            while (true) {
                try {
                	...
                	while(true) {
                		int startloc = dataIn.indexOf("$FSTART$");
                		if(startloc!=-1) {
                			int endloc = dataIn.indexOf("$FEND$", startloc);
                			if(endloc!=-1) {
                				String astring = dataIn.substring(startloc+8, endloc);
                		...
                		//if(bytes > 0) {
                	//		mHandler.obtainMessage(btComm.MESSAGE_TOAST, bytes, -1, buffer).sendToTarget();
                			
                		//}
                	}
                     //Send the obtained bytes to the UI Activity
                    mHandler.obtainMessage(btComm.MESSAGE_DATA_IN, -1, -1, dataIn.toString()).sendToTarget();
                } catch (IOException e) {
            ...
            try {
            	if(buffer.length > 50000) {
	                ...
	                while(index < buffer.length){
	                	mmOutStream.write(buffer, index, Math.min(buffer.length - index, step));
	                	index += step;
	                	try {
	                		Thread.sleep(2);
